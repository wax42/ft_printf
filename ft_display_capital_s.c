...
/*   Updated: 2018/01/11 22:51:35 by vguerand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_display_capital_s(va_list *ap, p_f parse)
{
	...
	if (parse.type != 'S')
		...
		if (str1 != NULL)
		{
			parse.val_ret = aff_struct((int)ft_wstrlen_byte(str1, parse.precision.width), &parse);
			parse.val_ret = ft_display_wchar(str1, parse);
			if (parse.val_ret == -1)
				return (-1);
			if (parse.neg.val && parse.precision.width <= parse.neg.width)
				parse.val_ret += ft_display_c(find_nbr(parse, parse.neg.width  , (int)ft_wstrlen_byte(str1, parse.precision.width)), 32);
		}
		else
		{
