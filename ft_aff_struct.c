...
/*   Updated: 2018/01/12 18:23:58 by vguerand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (parse->precision.val && parse->zero.val && !parse->plus && parse->type != 's' && parse->type != 'S' )
	{
		parse->width.val = 1;
	...
	if ((parse->width.val == 1) || (parse->zero.val == 1 && parse->neg.val == 0 && parse->precision.val == 0))
		parse->val_ret += ft_display_c(find_nbr(*parse, parse->width.width - parse->htag.width, str), 32);
	...
	if (parse->zero.val == 1 && parse->neg.val == 0 && (parse->precision.val == 0 || parse->type == 's' || parse->type == 'S'))
		parse->val_ret += ft_display_c(find_nbr(*parse, parse->zero.width - parse->space.val - parse->htag.width + parse->plus, str), '0');
